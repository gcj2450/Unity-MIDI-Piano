...
public class GridAndLoop : MonoBehaviour
{
    ...
    {
        InitList();
        onInitializeItem += UpdateItemContent;  //测试：用于动态更新Items内容
    }

    int PerPageCnt = 4;
    int curIndex = 0;
    private void UpdateItemContent(GameObject item, int wrapIndex, int realIndex)
    {
        if (OnUpdateItem != null)
            OnUpdateItem(item,realIndex);
        //item.transform.Find("Text").GetComponent<Text>().text = wrapIndex + "_" + realIndex;
        //item.transform.Find("Text").GetComponent<Text>().fontSize = 32;
        curIndex = realIndex;
    }
    public int getPageId()
    {
        return Mathf.CeilToInt( curIndex / PerPageCnt * 1.0f);
    }
    public int getTotalPage()
    {
        int PagCnt = Mathf.CeilToInt(maxIndex / PerPageCnt * 1.0f);
        Debug.Log(PerPageCnt);
        return PagCnt;
    }
    ...
    void WrapContent()
    {
        ...
        if (mHorizontal)
        {
            ...
            for (int i = 0, imax = mChild.Count; i < imax; i++)
            {
                ...
                {
                    Vector2 pos=temp.localPosition;
                    bool tempState = pos.x > min && pos.x < max;
                    temp.gameObject.SetActive(tempState);
        ...
        else 
        {
            ...
            for (int i = 0, imax = mChild.Count; i < imax; i++)
            {
                ...
                {
                    Vector2 pos = temp.localPosition;
                    bool tempState = pos.y > min && pos.y < max;
                    temp.gameObject.SetActive(tempState);
                }
