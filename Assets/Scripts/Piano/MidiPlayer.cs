...
public class MidiPlayer : MonoBehaviour
{
            ...
            while (_noteIndex < MidiNotes.Length && MidiNotes[_noteIndex].StartTime < _timer)
            {
                if (PianoKeyDetector.PianoNotes.ContainsKey(MidiNotes[_noteIndex].Note))
                {
                    if (ShowMIDIChannelColours)
                    {
                        ...
                        if (MidiNotes[_noteIndex].Channel < MIDIChannelColours.Length)
                        {
    ...
    public SongControl songCtrl;
    //PhotoScore识别谱转为midi或者musicxml
    void PlayCurrentMIDI()
    {
        ...
        _noteIndex = 0;
        OnPlayTrack.Invoke();
        songCtrl.LoadSongToGame(MidiNotes, _midi.GetTempo(), MIDISongs[_midiIndex].Speed);
