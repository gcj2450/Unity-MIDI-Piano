...
public class MidiRenderer : MonoBehaviour
{
    ...
    List<SmfLiteExtension.MidiNote> notes;
    ...
    void LoadMidiNotes()
    {
        ...
        notes = new List<SmfLiteExtension.MidiNote>();
        Debug.Log("division: " + song.division);
        var trackArray = song.tracks.ToArray();

        var draftContainer = new List<SmfLiteExtension.MidiNote>();
        var timeLine = 0;
        ...
        foreach (var track in trackArray)
        {
            foreach (var eventPair in track)
            {
                ...
                if (eventOn.Value)
                {
                    var note = new SmfLiteExtension.MidiNote()
                    {
    ...
    void DispatchEvents(List<SmfLiteExtension.MidiNote> notes)
    {
        if (notes != null)
        {
